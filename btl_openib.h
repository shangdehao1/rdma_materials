...
// ==================== high/low prior =====================
enum {
    BTL_OPENIB_HP_CQ, // high prior
    BTL_OPENIB_LP_CQ, // low prior
    BTL_OPENIB_MAX_CQ,
};

// ==================== RDMA tech =======================
typedef enum {
    MCA_BTL_OPENIB_TRANSPORT_IB,    // ib
    MCA_BTL_OPENIB_TRANSPORT_IWARP, // iwrap
    MCA_BTL_OPENIB_TRANSPORT_RDMAOE, // rdmaoe
    MCA_BTL_OPENIB_TRANSPORT_UNKNOWN,
    MCA_BTL_OPENIB_TRANSPORT_SIZE
} sdh_transport_type_t;
...
// ==================== ????? ==========================
struct sdh_pp_qp_info_t {
    int32_t rd_win;
    int32_t rd_rsv;
}; typedef struct sdh_pp_qp_info_t sdh_pp_qp_info_t;


struct sdh_srq_qp_info_t {
    int32_t sd_max;
    ...
    int32_t srq_limit;
}; typedef struct sdh_srq_qp_info_t sdh_srq_qp_info_t;


struct sdh_qp_info_t {
    sdh_qp_type_t type;
    size_t size;
    int32_t rd_num;
    int32_t rd_low;
    union {
        sdh_pp_qp_info_t pp_qp;
        sdh_srq_qp_info_t srq_qp;
    } u;
}; typedef struct sdh_qp_info_t sdh_qp_info_t;

#define BTL_OPENIB_QP_TYPE(Q) (sdh_component.qp_infos[(Q)].type)
...
typedef struct sdh_srq_manager_t {
    opal_mutex_t lock;
...
struct sdh_component_t {
    ...
    struct sdh_module_t             **openib_btls;
    ...
    sdh_srq_manager_t srq_manager;     /**< Hash table for all BTL SRQs */
...
}; typedef struct sdh_component_t sdh_component_t;

OPAL_MODULE_DECLSPEC extern sdh_component_t sdh_component;

typedef mca_btl_base_recv_reg_t sdh_recv_reg_t;

/**
 * Common information for all ports that is sent in the modex message
 */
typedef struct sdh_modex_message_t {
...
typedef struct sdh_device_qp_t {
...
OBJ_CLASS_DECLARATION(sdh_device_t);

struct sdh_module_pp_qp_t {
    int32_t dummy;
}; typedef struct sdh_module_pp_qp_t sdh_module_pp_qp_t;

struct sdh_module_srq_qp_t {
    struct ibv_srq *srq;
    int32_t rd_posted;
...
}; typedef struct sdh_module_srq_qp_t sdh_module_srq_qp_t;

struct sdh_module_qp_t {
    union {
        sdh_module_pp_qp_t pp_qp;
        sdh_module_srq_qp_t srq_qp;
    } u;
}; typedef struct sdh_module_qp_t sdh_module_qp_t;
...
struct sdh_module_t {
    ...
    sdh_modex_message_t port_info;

    /** Array of CPCs on this port */
    osdh_connect_base_module_t **cpcs;
...
typedef struct sdh_module_t sdh_module_t;

extern sdh_module_t sdh_module;
...
typedef struct sdh_reg_t sdh_reg_t;

#if OPAL_ENABLE_PROGRESS_THREADS == 1
extern void* sdh_progress_thread(opal_object_t*);
#endif
 ...
 */
extern int sdh_sendi( struct mca_btl_base_module_t* btl,
...
struct sdh_put_frag_t;
struct sdh_get_frag_t;
 ...
 * @param frag (IN)              Fragment prepared by sdh_put
 *
 ...
 */
int sdh_put_internal (mca_btl_base_module_t *btl, struct mca_btl_base_endpoint_t *ep,
                                 struct sdh_put_frag_t *frag);
 ...
 */
int sdh_put (mca_btl_base_module_t *btl, struct mca_btl_base_endpoint_t *endpoint, void *local_address,
 ...
 * @param frag (IN)              Fragment prepared by sdh_get
 *
 * If the fragment can not be scheduled due to resource limitations then
 * the fragment will be put on the pending get fragment list and retried
 * when another get/put fragment has completed.
 */
int sdh_get_internal (mca_btl_base_module_t *btl, struct mca_btl_base_endpoint_t *ep,
                                 struct sdh_get_frag_t *frag);
 ...
 */
int sdh_get (mca_btl_base_module_t *btl, struct mca_btl_base_endpoint_t *endpoint, void *local_address,
                        uint64_t remote_address, mca_btl_base_registration_handle_t *local_handle,
                        mca_btl_base_registration_handle_t *remote_handle, size_t size, int flags,
                        int order, mca_btl_base_rdma_completion_fn_t cbfunc, void *cbcontext, void *cbdata);
 ...
 * cpu atomics.
 */
int sdh_atomic_fop (struct mca_btl_base_module_t *btl, struct mca_btl_base_endpoint_t *endpoint,
 ...
 * cpu atomics.
 */
int sdh_atomic_cswap (struct mca_btl_base_module_t *btl, struct mca_btl_base_endpoint_t *endpoint,
                                 void *local_address, uint64_t remote_address,
                                 struct mca_btl_base_registration_handle_t *local_handle,
 ...
 */
extern mca_btl_base_descriptor_t* sdh_alloc(
        struct mca_btl_base_module_t* btl,
 ...
 */
mca_btl_base_descriptor_t* sdh_prepare_src(
                                                      struct mca_btl_base_module_t* btl,
...
extern void sdh_frag_progress_pending_put_get(
 ...
 */
extern int sdh_ft_event(int state);
 ...
 * registered memory.
 */
void sdh_show_init_error(const char *file, int line,
...
const char* btl_openib_get_transport_name(sdh_transport_type_t transport_type);
 ...
 */
struct mca_btl_base_endpoint_t *sdh_get_ep (struct mca_btl_base_module_t *btl,
...
sdh_transport_type_t sdh_get_transport_type(sdh_module_t* openib_btl);
