...
// =================  Local variables: sdh_component =============================
//
static sdh_device_t *receive_queues_device = NULL;
static int num_devices_intentionally_ignored = 0;

sdh_component_t sdh_component = {
    .super = {
...
static int btl_openib_component_register(void)
{
    ...
    sdh_component.max_send_size = sdh_module.super.btl_max_send_size;
    sdh_component.eager_limit = sdh_module.super.btl_eager_limit;

    // if_include and if_exclude need to be mutually exclusive 
    if (OPAL_SUCCESS != mca_base_var_check_exclusive("ompi",
                                                    sdh_component.super.btl_version.mca_type_name,
                                                    sdh_component.super.btl_version.mca_component_name,
                                                    "if_include",
                                                    sdh_component.super.btl_version.mca_type_name,
                                                    sdh_component.super.btl_version.mca_component_name,
                                                    "if_exclude")) 
    {
       // a warning message about "open" failing is not printed 
...
static int btl_openib_component_open(void)
{
    opal_mutex_t *lock = &sdh_component.srq_manager.lock;
    opal_hash_table_t *srq_addr_table = &sdh_component.srq_manager.srq_addr_table;
    ...
    // initialize state 
    sdh_component.ib_num_btls = 0;
    sdh_component.num_default_gid_btls = 0;
    sdh_component.openib_btls = NULL;
    OBJ_CONSTRUCT(&sdh_component.devices, opal_pointer_array_t);
    sdh_component.devices_count = 0;
    sdh_component.cpc_explicitly_defined = false;

    // initialize objects 
    OBJ_CONSTRUCT(&sdh_component.ib_procs, opal_list_t);
    sdh_component.memory_registration_verbose = -1;
...
static int btl_openib_component_close(void)
{
    ...
    OBJ_DESTRUCT(&sdh_component.srq_manager.lock);
    OBJ_DESTRUCT(&sdh_component.srq_manager.srq_addr_table);

    opal_btl_openib_connect_base_finalize();
    opal_btl_openib_ini_finalize();

    if (NULL != sdh_component.default_recv_qps) 
    {
        free(sdh_component.default_recv_qps);
    }

    // close memory registration debugging output 
    opal_output_close (sdh_component.memory_registration_verbose);
...
static int btl_openib_modex_send(void)
{
    ...
    if (0 == sdh_component.ib_num_btls) {
        return 0;
    }
    modex_message_size = offsetof(sdh_modex_message_t, end);
    ...
    for (i = 0; i < sdh_component.ib_num_btls; i++) {
        for (j = 0;
             j < sdh_component.openib_btls[i]->num_cpcs;
             ++j) {
            msg_size +=
                ...
                1 +
                /* blob length */
                sdh_component.openib_btls[i]->cpcs[j]->data.cbm_modex_message_len;
        }
    }
    ...
    offset = message;
    pack8(&offset, sdh_component.ib_num_btls);
    ...
    for (i = 0; i < sdh_component.ib_num_btls; i++) {
        ...
        (sdh_component.openib_btls[i]->port_info).vendor_id =
            (sdh_component.openib_btls[i]->device->ib_dev_attr).vendor_id;

        (sdh_component.openib_btls[i]->port_info).vendor_part_id =
            (sdh_component.openib_btls[i]->device->ib_dev_attr).vendor_part_id;

        (sdh_component.openib_btls[i]->port_info).transport_type =
            sdh_get_transport_type(sdh_component.openib_btls[i]);

        memcpy(offset,
               &(sdh_component.openib_btls[i]->port_info),
               size);
        opal_output(-1, "modex packed btl port modex message: 0x%" PRIx64 ", %d, %d (size: %d)",
                    sdh_component.openib_btls[i]->port_info.subnet_id,
                    sdh_component.openib_btls[i]->port_info.mtu,
                    sdh_component.openib_btls[i]->port_info.lid,
                    (int) size);

#if !defined(WORDS_BIGENDIAN) && OPAL_ENABLE_HETEROGENEOUS_SUPPORT
        MCA_BTL_OPENIB_MODEX_MSG_HTON(*(sdh_modex_message_t *)offset);
#endif
        offset += size;
        ...
        pack8(&offset,
              sdh_component.openib_btls[i]->num_cpcs);
        opal_output(-1, "modex packed btl %d: to pack %d cpcs (packed: %d, offset now %d)",
                    i, sdh_component.openib_btls[i]->num_cpcs,
        ...
        for (j = 0;
            ...
            cpc = sdh_component.openib_btls[i]->cpcs[j];
    ...
    OPAL_MODEX_SEND(rc, OPAL_PMIX_GLOBAL,
                    &sdh_component.super.btl_version,
                    message, msg_size);
    free(message);
                               ...
                               void* cbdata)
{
    ...
    sdh_module_t *obtl = (sdh_module_t*)btl;
    sdh_endpoint_t* ep = to_com_frag(des)->endpoint;
    sdh_control_header_t *ctl_hdr =
        (sdh_control_header_t *) to_base_frag(des)->segment.seg_addr.pval;
    sdh_eager_rdma_header_t *rdma_hdr;

    sdh_header_coalesced_t *clsc_hdr = (sdh_header_coalesced_t*)(ctl_hdr + 1);
    ...
    switch (ctl_hdr->type) 
    {
        ...
        case MCA_BTL_OPENIB_CONTROL_RDMA:
            rdma_hdr = (sdh_eager_rdma_header_t*)ctl_hdr;
            ...
            ep->eager_rdma_remote.tokens=sdh_component.eager_rdma_num - 1;
            break;
        case MCA_BTL_OPENIB_CONTROL_COALESCED:
        {
            ...
            {
                size_t skip;
                sdh_header_coalesced_t* unalign_hdr = 0;
                ...
                len -= (skip + pad);
                unalign_hdr = (sdh_header_coalesced_t*)((unsigned char*)clsc_hdr + skip);
                pad = (size_t)BTL_OPENIB_COALESCE_HDR_PADDING(unalign_hdr);
                clsc_hdr = (sdh_header_coalesced_t*)((unsigned char*)unalign_hdr + pad);
            }
        }
            break;
        case MCA_BTL_OPENIB_CONTROL_CTS:
            ...
            if (ep->endpoint_posted_recvs) 
            {
                ...
                {
                    sdh_endpoint_send_cts(ep);
                }
                sdh_endpoint_connected(ep);
            }
            break;
        default:
...
static int openib_reg_mr (void *reg_data, void *base, size_t size, mca_rcache_base_registration_t *reg)
{
    sdh_device_t *device = (sdh_device_t*)reg_data;
    sdh_reg_t *openib_reg = (sdh_reg_t*)reg;
    ...
    if (device->mem_reg_max && device->mem_reg_max < (device->mem_reg_active + size)) 
    {
        return OPAL_ERR_OUT_OF_RESOURCE;
    }
...
static int init_one_port(opal_list_t *btl_list, sdh_device_t *device,
                         ...
                         struct ibv_port_attr *ib_port_attr)
{
    ...
    if (IBV_TRANSPORT_IWARP == device->ib_dev->transport_type) {
        ...
        if (IBV_LINK_LAYER_ETHERNET == ib_port_attr->link_layer) {
            subnet_id = sdh_component.rroce_enable ? 0 :
                   sdh_get_ip_subnet_id(device->ib_dev, port_num);
        } else {
    ...
    if(sdh_component.num_default_gid_btls > 0 &&
            IB_DEFAULT_GID_PREFIX == subnet_id &&
            sdh_component.warn_default_gid_prefix) {
        opal_show_help("help-mpi-btl-openib.txt", "default subnet prefix",
                true, opal_process_info.nodename);
    }

    if (IB_DEFAULT_GID_PREFIX == subnet_id) {
        sdh_component.num_default_gid_btls++;
    }

    lmc = (1 << ib_port_attr->lmc);
    lmc_step = 1;

    if (0 != sdh_component.max_lmc &&
        sdh_component.max_lmc < lmc) {
        lmc = sdh_component.max_lmc;
    }
    ...
    /* APM support -- only meaningful if async event support is
       ...
       worth enabling APM.  */
    if (lmc > 1){
        if (-1 == sdh_component.apm_lmc) {
            lmc_step = lmc;
            sdh_component.apm_lmc = lmc - 1;
        } else if (0 == lmc % (sdh_component.apm_lmc + 1)) {
            lmc_step = sdh_component.apm_lmc + 1;
        } else {
            opal_show_help("help-mpi-btl-openib.txt", "apm with wrong lmc",true,
                    sdh_component.apm_lmc, lmc);
            return OPAL_ERROR;
        }
    } else {
        if (sdh_component.apm_lmc) {
            /* Disable apm and report warning */
            sdh_component.apm_lmc = 0;
    ...
    for(lid = ib_port_attr->lid;
        ...
        for(i = 0; i < sdh_component.btls_per_lid; i++){
            char param[40];

            openib_btl = (sdh_module_t *) calloc(1, sizeof(sdh_module_t));
            if(NULL == openib_btl) {
                BTL_ERROR(("Failed malloc: %s:%d", __FILE__, __LINE__));
                return OPAL_ERR_OUT_OF_RESOURCE;
            }
            memcpy(openib_btl, &sdh_module,
                    sizeof(sdh_module));
           ...
           param_register_uint(param, openib_btl->super.btl_latency, &openib_btl->super.btl_latency);
            ...
            ++device->btls;
            ++sdh_component.ib_num_btls;
            if (-1 != sdh_component.ib_max_btls &&
                sdh_component.ib_num_btls >=
                sdh_component.ib_max_btls) {
...
static void device_construct(sdh_device_t *device)
{
    device->ib_dev = NULL;
    ...
    device->hp_cq_polls = sdh_component.cq_poll_ratio;
    device->eager_rdma_polls = sdh_component.eager_rdma_poll_ratio;
    device->pollme = true;
...
static void device_destruct(sdh_device_t *device)
{
    int i;
    ...
    if (NULL != device->qps) {
        for (i = 0; i < sdh_component.num_qps; i++) {
    ...
    if (MCA_BTL_XRC_ENABLED) {
        if (OPAL_SUCCESS != sdh_close_xrc_domain(device)) {
...
OBJ_CLASS_INSTANCE(sdh_device_t, opal_object_t, device_construct,
        device_destruct);

static int
get_port_list(sdh_device_t *device, int *allowed_ports)
{
    ...
    num_ports = 0;
    if (NULL != sdh_component.if_include_list) {
        /* If only the device name is given (eg. mtdevice0,mtdevice1) use all
           ports */
        i = 0;
        while (sdh_component.if_include_list[i]) {
            if (0 == strcmp(dev_name,
                            sdh_component.if_include_list[i])) {
        ...
        for (i = 1; i <= device->ib_dev_attr.phys_port_cnt; ++i) {
            ...
            for (j = 0;
                 NULL != sdh_component.if_include_list[j]; ++j) {
                if (0 == strcmp(name,
                                sdh_component.if_include_list[j])) {
                    allowed_ports[num_ports++] = i;
                    break;
                }
            }
        }
    } else if (NULL != sdh_component.if_exclude_list) {
        /* If only the device name is given (eg. mtdevice0,mtdevice1) exclude
           all ports */
        i = 0;
        while (sdh_component.if_exclude_list[i]) {
            if (0 == strcmp(dev_name,
                            sdh_component.if_exclude_list[i])) {
                num_ports = 0;
                goto done;
            }
            ++i;
        ...
        for (i = 1; i <= device->ib_dev_attr.phys_port_cnt; ++i) {
            sprintf(name,"%s:%d",dev_name,i);
            for (j = 0;
                 NULL != sdh_component.if_exclude_list[j]; ++j) {
                if (0 == strcmp(name,
                                sdh_component.if_exclude_list[j])) {
    ...
    if (NULL != sdh_component.if_list) {
        for (i = 0; NULL != sdh_component.if_list[i]; ++i) {

            /* Look for raw device name */
            if (0 == strcmp(sdh_component.if_list[i], dev_name)) {
                j = opal_argv_count(sdh_component.if_list);
                opal_argv_delete(&j, &(sdh_component.if_list),
                                 i, 1);
                --i;
            }
        }
        for (i = 1; i <= device->ib_dev_attr.phys_port_cnt; ++i) {
            sprintf(name, "%s:%d", dev_name, i);
            for (j = 0; NULL != sdh_component.if_list[j]; ++j) {
                if (0 == strcmp(sdh_component.if_list[j], name)) {
                    k = opal_argv_count(sdh_component.if_list);
                    opal_argv_delete(&k, &(sdh_component.if_list),
                                     j, 1);
                    --j;
                    break;
                }
...
static bool inline is_credit_message(const sdh_recv_frag_t *frag)
{
    sdh_control_header_t* chdr =
        (sdh_control_header_t *) to_base_frag(frag)->segment.seg_addr.pval;
    return (MCA_BTL_TAG_IB == frag->hdr->tag) &&
        (MCA_BTL_OPENIB_CONTROL_CREDITS == chdr->type);
}

static bool inline is_cts_message(const sdh_recv_frag_t *frag)
{
    sdh_control_header_t* chdr =
        (sdh_control_header_t *) to_base_frag(frag)->segment.seg_addr.pval;
    return (MCA_BTL_TAG_IB == frag->hdr->tag) &&
...
static void init_apm_port(sdh_device_t *device, int port, uint16_t lid)
{
    int index;
    struct sdh_module_t *btl;
    for(index = 0; index < device->btls; index++) {
        btl = (sdh_module_t *) opal_pointer_array_get_item(device->device_btls, index);
        /* Ok, we already have btl for the fist port,
         * second one will be used for APM */
        btl->apm_port = port;
        btl->port_info.apm_lid = lid + btl->src_path_bits;
        sdh_component.apm_ports++;
...
static int setup_qps(void)
{
    ...
    queues = opal_argv_split(sdh_component.receive_queues, ':');
    if (0 == opal_argv_count(queues)) {
        opal_show_help("help-mpi-btl-openib.txt",
                       "no qps in receive_queues", true,
                       opal_process_info.nodename,
                       sdh_component.receive_queues);
        ret = OPAL_ERROR;
        goto error;
    }
    ...
    if (num_xrc_qps > 0 && sdh_component.btls_per_lid > 1) {
        opal_show_help("help-mpi-btl-openib.txt", "XRC with BTLs per LID",
                       true, opal_process_info.nodename,
                       sdh_component.receive_queues, num_xrc_qps);
        ret = OPAL_ERR_BAD_PARAM;
        goto error;
    }
#endif

    sdh_component.num_pp_qps = num_pp_qps;
    sdh_component.num_srq_qps = num_srq_qps;
    sdh_component.num_xrc_qps = num_xrc_qps;
    sdh_component.num_qps = num_pp_qps + num_srq_qps + num_xrc_qps;

    sdh_component.qp_infos = (sdh_qp_info_t*)
        malloc(sizeof(sdh_qp_info_t) *
                sdh_component.num_qps);
    if (NULL == sdh_component.qp_infos) {
...
#define P(N) (((N) > count) ? NULL : params[(N)])
    while (queues[qp] != NULL) {
        ...
        if ('P' == params[0][0]) {
            ...
            sdh_component.qp_infos[qp].type = MCA_BTL_OPENIB_PP_QP;
            sdh_component.qp_infos[qp].size = atoi_param(P(1), 0);
            ...
            sdh_component.qp_infos[qp].u.pp_qp.rd_win = rd_win;
            sdh_component.qp_infos[qp].u.pp_qp.rd_rsv = rd_rsv;
            ...
            sdh_component.qp_infos[qp].type = (params[0][0] =='X') ?
            ...
            sdh_component.qp_infos[qp].u.srq_qp.sd_max = sd_max;
            sdh_component.qp_infos[qp].u.srq_qp.rd_init = rd_init;
            sdh_component.qp_infos[qp].u.srq_qp.srq_limit = srq_limit;
        }
        ...
        sdh_component.qp_infos[qp].rd_num = rd_num;
        sdh_component.qp_infos[qp].rd_low = rd_low;
    ...
    max_qp_size = sdh_component.qp_infos[sdh_component.num_qps - 1].size;
    max_size_needed = (sdh_module.super.btl_eager_limit >
                       sdh_module.super.btl_max_send_size) ?
        sdh_module.super.btl_eager_limit :
        sdh_module.super.btl_max_send_size;
    ...
    if (max_qp_size < max_size_needed) {
        ...
        if (sdh_module.super.btl_eager_limit > max_qp_size &&
            MCA_BASE_VAR_SOURCE_DEFAULT == eager_source) {
            sdh_module.super.btl_eager_limit = max_qp_size;
        }

        if (sdh_module.super.btl_max_send_size > max_qp_size &&
            MCA_BASE_VAR_SOURCE_DEFAULT == max_send_source) {
            sdh_module.super.btl_max_send_size = max_qp_size;
        }
    ...
    if (sdh_component.ib_free_list_max > 0 &&
        min_freelist_size > sdh_component.ib_free_list_max) {
        opal_show_help("help-mpi-btl-openib.txt", "freelist too small", true,
                       opal_process_info.nodename,
                       sdh_component.ib_free_list_max,
                       min_freelist_size);
        ret = OPAL_ERR_BAD_PARAM;
        goto error;
    }

    sdh_component.rdma_qp = sdh_component.num_qps - 1;
    if (sdh_component.num_qps > smallest_pp_qp) {
        sdh_component.credits_qp = smallest_pp_qp;
    } else {
        sdh_component.credits_qp = sdh_component.num_qps - 1;
    }
...
static uint64_t calculate_max_reg (const char *device_name)
{
    ...
    if (sdh_component.allow_max_memory_registration) {
        max_reg = 2 * mem_total;
    ...
    if (max_reg < mem_total * 3 / 4) {
        ...
        if (sdh_component.abort_not_enough_reg_mem) {
...
static int init_one_device(opal_list_t *btl_list, struct ibv_device* ib_dev)
{
    ...
    if(( 0 == device->mem_reg_max) && sdh_component.abort_not_enough_reg_mem) {
        return OPAL_ERROR;
    }
    ...
    if (OPAL_ERR_NOT_FOUND == ret) {
        ...
        if (sdh_component.warn_no_device_params_found) {
            opal_show_help("help-mpi-btl-openib.txt",
    ...
    if (0 < sdh_component.ib_mtu) {
        device->mtu = sdh_component.ib_mtu;
    ...
    need_search = false;
    if(-2 != sdh_component.ib_max_inline_data) {
        /* User has explicitly set btl_openib_max_inline_data MCA parameter
           ...
           to be >= -1 */
        if (-1 == sdh_component.ib_max_inline_data) {
            need_search = true;
        } else {
            device->max_inline_data = (uint32_t)
                sdh_component.ib_max_inline_data;
        }
    ...
    if (sdh_component.use_eager_rdma >= 0) {
        device->use_eager_rdma = sdh_component.use_eager_rdma;
    ...
    rcache_resources.sizeof_reg = sizeof(sdh_reg_t);
    rcache_resources.register_mem = openib_reg_mr;
    rcache_resources.deregister_mem = openib_dereg_mr;
    device->rcache =
        mca_rcache_base_module_create (sdh_component.ib_rcache_name,
    ...
    device->mpool = mca_mpool_base_module_lookup (sdh_component.ib_mpool_hints);
    ...
    for(k = 0; k < port_cnt; k++){
        ...
        if(IBV_PORT_ACTIVE == ib_port_attr.state) {
            ...
            if (sdh_component.apm_ports && device->btls > 0) {
                init_apm_port(device, i, ib_port_attr.lid);
                break;
            }
            if (0 == sdh_component.ib_pkey_val) {
                ...
                for (j = 0; j < device->ib_dev_attr.max_pkeys; j++) {
                    ...
                    if(pkey == sdh_component.ib_pkey_val){
    ...
    if (device->btls > 0) {
        ...
        if (1 == sdh_component.apm_ports) {
            opal_show_help("help-mpi-btl-openib.txt",
                           "apm not enough ports", true);
            sdh_component.apm_ports = 0;
        }
        ...
        /* Check to ensure that all devices used in this process have
           ...
           sdh_receive_queues value, the short description
        ...
        */

        {
            ...
            sdh_component.receive_queues_source = source;
        }

        /* If the MCA param was specified, skip all the checks */
        if (MCA_BASE_VAR_SOURCE_DEFAULT != sdh_component.receive_queues_source) {
            goto good;
        }
        ...
        /* If we're the first device and we have a receive_queues
           ...
           use that. */
        if (0 == sdh_component.devices_count) {
            if (NULL != values.receive_queues &&
                0 != strcmp(values.receive_queues,
                            sdh_component.receive_queues)) {
                if (NULL != sdh_component.receive_queues) {
                    free(sdh_component.receive_queues);
                }
                sdh_component.receive_queues =
                    strdup(values.receive_queues);
                sdh_component.receive_queues_source =
            ...
            /* If this device doesn't have an INI receive_queues
               ...
               device's INI file, we must error. */
                ...
                opal_show_help("help-mpi-btl-openib.txt",
                               ...
                               device->ib_dev_attr.vendor_id,
                               device->ib_dev_attr.vendor_part_id,
                               sdh_component.default_recv_qps);
                ret = OPAL_ERR_RESOURCE_BUSY;
                goto error;
            }
        }
...
static int finish_btl_init(sdh_module_t *openib_btl)
{
    int qp;
    ...
    openib_btl->qps = (sdh_module_qp_t*)
        calloc(sdh_component.num_qps,
                sizeof(sdh_module_qp_t));
    ...
    for (qp = 0; qp < sdh_component.num_qps; qp++) {
        if (!BTL_OPENIB_QP_TYPE_PP(qp)) {
            ...
            openib_btl->qps[qp].u.srq_qp.sd_credits =
                sdh_component.qp_infos[qp].u.srq_qp.sd_max;
    ...
    openib_btl->eager_rdma_frag_size = OPAL_ALIGN(
            ...
            sdh_component.buffer_alignment, size_t);
...
static float get_ib_dev_distance(struct ibv_device *dev)
{
    ...
    if (0 != sdh_component.ignore_locality ||
                          ...
                          bool enable_mpi_threads)
{
    ...
    sdh_frag_init_data_t *init_data;
    ...
    *num_btl_modules = 0;
     ...
     */
    opal_memory->memoryc_set_alignment(32, sdh_module.super.btl_eager_limit);
    ...
    if (index >= 0) {
        if (OPAL_SUCCESS == mca_base_var_get_value(index, NULL, &source, NULL)) {
            if (-1 == sdh_component.ib_max_inline_data  &&
                ...
                sdh_component.ib_max_inline_data = -2;
            }
        }
    }

    OBJ_CONSTRUCT(&sdh_component.send_free_coalesced, opal_free_list_t);
    OBJ_CONSTRUCT(&sdh_component.send_user_free, opal_free_list_t);
    OBJ_CONSTRUCT(&sdh_component.recv_user_free, opal_free_list_t);

    init_data = (sdh_frag_init_data_t *) malloc(sizeof(sdh_frag_init_data_t));
    if (NULL == init_data) {
        BTL_ERROR(("Failed malloc: %s:%d", __FILE__, __LINE__));
        goto no_btls;
    }

    init_data->order = sdh_component.rdma_qp;
    init_data->list = &sdh_component.send_user_free;
    ...
    if (OPAL_SUCCESS != opal_free_list_init (
                &sdh_component.send_user_free,
                sizeof(sdh_put_frag_t), 8,
                OBJ_CLASS(sdh_put_frag_t),
                0, 0,
                sdh_component.ib_free_list_num,
                sdh_component.ib_free_list_max,
                sdh_component.ib_free_list_inc,
                NULL, 0, NULL, sdh_frag_init, init_data)) {
        goto no_btls;
    }
    ...
    init_data->order = sdh_component.rdma_qp;
    init_data->list = &sdh_component.recv_user_free;

    if(OPAL_SUCCESS != opal_free_list_init (
                &sdh_component.recv_user_free,
                sizeof(sdh_get_frag_t), 8,
                OBJ_CLASS(sdh_get_frag_t),
                0, 0,
                sdh_component.ib_free_list_num,
                sdh_component.ib_free_list_max,
                sdh_component.ib_free_list_inc,
    ...
    length = sizeof(sdh_coalesced_frag_t);

    init_data->list = &sdh_component.send_free_coalesced;

    if(OPAL_SUCCESS != opal_free_list_init (
                &sdh_component.send_free_coalesced,
                length, 8, OBJ_CLASS(sdh_coalesced_frag_t),
                0, 0, sdh_component.ib_free_list_num,
                sdh_component.ib_free_list_max,
                sdh_component.ib_free_list_inc,
                NULL, 0, NULL, sdh_frag_init, init_data)) {
        goto no_btls;
    ...
    sdh_component.if_include_list =
        sdh_component.if_exclude_list =
        sdh_component.if_list = NULL;

    if (NULL != sdh_component.if_include)
      list_count++;
    if (NULL != sdh_component.if_exclude)
      list_count++;
    if (NULL != sdh_component.ipaddr_include)
      list_count++;
    if (NULL != sdh_component.ipaddr_exclude)
      list_count++;

    if (list_count > 1) {
        opal_show_help("help-mpi-btl-openib.txt",
                       "specified include and exclude", true,
                       NULL == sdh_component.if_include ?
                        "<not specified>" : sdh_component.if_include,
                       NULL == sdh_component.if_exclude ?
                        "<not specified>" : sdh_component.if_exclude,
                       NULL == sdh_component.ipaddr_include ?
                        "<not specified>" :sdh_component.ipaddr_include,
                       NULL == sdh_component.ipaddr_exclude ?
                         "<not specified>" :sdh_component.ipaddr_exclude,
                       NULL);
        goto no_btls;
    } else if (NULL != sdh_component.if_include) {
        sdh_component.if_include_list =
            opal_argv_split(sdh_component.if_include, ',');
        sdh_component.if_list =
            opal_argv_copy(sdh_component.if_include_list);
    } else if (NULL != sdh_component.if_exclude) {
        sdh_component.if_exclude_list =
            opal_argv_split(sdh_component.if_exclude, ',');
        sdh_component.if_list =
            opal_argv_copy(sdh_component.if_exclude_list);
    }
    ...
    OBJ_CONSTRUCT(&sdh_component.ib_lock, opal_mutex_t);

    distance = dev_sorted[0].distance;
    for (found = false, i = 0;
         i < num_devs && (-1 == sdh_component.ib_max_btls ||
                sdh_component.ib_num_btls <
                sdh_component.ib_max_btls); i++) {
        if (0 != sdh_component.ib_num_btls &&
        ...
        switch (sdh_component.device_type) {
    ...
    if (!found) {
        opal_show_help("help-mpi-btl-openib.txt", "no devices right type",
                       true, opal_process_info.nodename,
                       ((BTL_OPENIB_DT_IB == sdh_component.device_type) ?
                        "InfiniBand" :
                        (BTL_OPENIB_DT_IWARP == sdh_component.device_type) ?
                        "iWARP" : "<any>"));
        goto no_btls;
    }
    ...
    if (0 != opal_argv_count(sdh_component.if_list) &&
        sdh_component.warn_nonexistent_if) {
        char *str = opal_argv_join(sdh_component.if_list, ',');
    ...
    if (sdh_component.num_srq_qps > 0 ||
                     sdh_component.num_xrc_qps > 0) {
        opal_hash_table_t *srq_addr_table = &sdh_component.srq_manager.srq_addr_table;
        if(OPAL_SUCCESS != opal_hash_table_init(
                srq_addr_table, (sdh_component.num_srq_qps +
                                 sdh_component.num_xrc_qps) *
                                 sdh_component.ib_num_btls)) {
    ...
    if (MCA_BTL_XRC_ENABLED) {
        OBJ_CONSTRUCT(&sdh_component.ib_addr_table,
                opal_hash_table_t);
    }

    /* Allocate space for btl modules */
    sdh_component.openib_btls =
        (sdh_module_t **) malloc(sizeof(sdh_module_t*) *
                sdh_component.ib_num_btls);
    if(NULL == sdh_component.openib_btls) {
        BTL_ERROR(("Failed malloc: %s:%d", __FILE__, __LINE__));
        goto no_btls;
    }
    btls = (struct mca_btl_base_module_t **)
        malloc(sdh_component.ib_num_btls *
    ...
    while (NULL != (item = opal_list_remove_first(&btl_list))) {
        ...
        openib_btl = (sdh_module_t*)ib_selected->btl_module;
        ...
        if (sdh_component.max_hw_msg_size > 0 &&
            (uint32_t)sdh_component.max_hw_msg_size > openib_btl->ib_port_attr.max_msg_sz) {
            BTL_ERROR(("max_hw_msg_size (%" PRIu32 ") is larger than hw max message size (%" PRIu32 ")",
                sdh_component.max_hw_msg_size, openib_btl->ib_port_attr.max_msg_sz));
        }

        sdh_component.openib_btls[i] = openib_btl;
    ...
    /* Otherwise reset to the number of openib modules that we
       actually got */
    sdh_component.ib_num_btls = i;

    btl_openib_modex_send();

    *num_btl_modules = sdh_component.ib_num_btls;
    opal_ibv_free_device_list(ib_devs);
    if (NULL != sdh_component.if_include_list) {
        opal_argv_free(sdh_component.if_include_list);
        sdh_component.if_include_list = NULL;
    }
    if (NULL != sdh_component.if_exclude_list) {
        opal_argv_free(sdh_component.if_exclude_list);
        sdh_component.if_exclude_list = NULL;
    }

#if OPAL_CUDA_SUPPORT
   if (sdh_component.cuda_want_gdr && (0 == opal_leave_pinned)) {
        opal_show_help("help-mpi-btl-openib.txt",
    ...
    sdh_component.memory_registration_verbose = opal_output_open(NULL);
    opal_output_set_verbosity (sdh_component.memory_registration_verbose,
                               sdh_component.memory_registration_verbose_level);
            ...
            */
            for (len = opal_list_get_size(&ep->qps[qp].no_credits_pending_frags[pri]);
                ...
                /* If _endpoint_post_send() fails because of
                   ...
                   error upward. */
                rc = sdh_endpoint_post_send(ep, to_send_frag(frag));
...
void sdh_frag_progress_pending_put_get(mca_btl_base_endpoint_t *ep,
        const int qp)
{
    ...
    for(i = 0; i < len && ep->qps[qp].qp->sd_wqe > 0 && ep->get_tokens > 0; i++) {
        ...
        if (NULL == frag)
            break;
        rc = sdh_get_internal ((mca_btl_base_module_t *)openib_btl, ep,
                                          to_get_frag(frag));
    ...
    for(i = 0; i < len && ep->qps[qp].qp->sd_wqe > 0; i++) {
        ...
        if (NULL == frag)
            break;
        rc = sdh_put_internal ((mca_btl_base_module_t*)openib_btl, ep,
                                          to_put_frag(frag));
        if (OPAL_ERR_OUT_OF_RESOURCE == rc) {
            OPAL_THREAD_LOCK(&ep->endpoint_lock);
...
static int btl_openib_handle_incoming(sdh_module_t *openib_btl,
                                         sdh_endpoint_t *ep,
                                         sdh_recv_frag_t *frag,
                                         size_t byte_len)
{
     ...
     * length.*/
    des->des_segments->seg_len = byte_len - sizeof(sdh_header_t);
    ...
    if(OPAL_LIKELY(!(is_credit_msg = is_credit_message(frag)))) {
        ...
        sdh_rdma_credits_header_t *chdr =
            (sdh_rdma_credits_header_t *) des->des_segments->seg_addr.pval;
        if(ep->nbo) {
    ...
    if(MCA_BTL_OPENIB_RDMA_FRAG(frag)) {
        sdh_eager_rdma_local_t *erl = &ep->eager_rdma_local;
        ...
        if (is_cts_message(frag)) {
            ...
            if (BTL_OPENIB_QP_TYPE_PP(rqp)) {
                ...
                sdh_endpoint_post_rr(ep, cqp);
            } else {
                sdh_module_t *btl = ep->endpoint_btl;
                OPAL_THREAD_ADD_FETCH32(&btl->qps[rqp].u.srq_qp.rd_posted, -1);
                sdh_post_srr(btl, rqp);
            }
        }
    }
...
static void btl_openib_handle_incoming_completion(mca_btl_base_module_t* btl,
                                                 ...
                                                 int status)
{
    sdh_recv_frag_t *frag = (sdh_recv_frag_t *)des;
    sdh_header_t *hdr = frag->hdr;
    int rqp = to_base_frag(frag)->base.order, cqp;
    uint16_t rcredits = 0, credits;
    ...
    if (rcredits + credits > 0) {
        ...
        if (OPAL_SUCCESS !=
            ...
            sdh_module_t* openib_btl = (sdh_module_t*) btl;
...
progress_pending_frags_wqe(mca_btl_base_endpoint_t *ep, const int qpn)
{
    ...
    for(int i = 0; i < 2; i++) {
       while(qp->sd_wqe > 0) {
            ...
            ret = sdh_endpoint_post_send(tmp_ep, to_send_frag(frag));
            if (OPAL_SUCCESS != ret) {
...
static void progress_pending_frags_srq(sdh_module_t* openib_btl,
        const int qp)
{
    opal_list_item_t *frag;
    int i;
    ...
    for(i = 0; i < 2; i++) {
        while(openib_btl->qps[qp].u.srq_qp.sd_credits > 0) {
            ...
            sdh_endpoint_send(to_com_frag(frag)->endpoint,
                    to_send_frag(frag));
        }
    }
}
...
static void handle_wc(sdh_device_t* device, const uint32_t cq, struct ibv_wc *wc)
{
    static int flush_err_printed[] = {0, 0};
    sdh_com_frag_t* frag;
    mca_btl_base_descriptor_t *des;
    sdh_endpoint_t* endpoint;
    sdh_module_t *openib_btl = NULL;
    ...
    // For receive fragments "order" contains QP idx the fragment was posted to. 
    // For send fragments "order" contains QP idx the fragment was send through 
    qp = des->order;
...
#endif
        frag->endpoint = (sdh_endpoint_t*)opal_pointer_array_get_item(device->endpoints, wc->imm_data);
    }
    ...
    switch(wc->opcode) 
    {
        ...
        case IBV_WC_FETCH_ADD:
            ...
            sdh_get_frag_t *get_frag = to_get_frag(des);
        ...
        case IBV_WC_RDMA_WRITE:
            ...
            {
                sdh_put_frag_t *put_frag = to_put_frag(des);
        ...
        case IBV_WC_SEND:
            if(openib_frag_type(des) == MCA_BTL_OPENIB_FRAG_SEND) 
            {
                ...
                while((i = opal_list_remove_first(&to_send_frag(des)->coalesced_frags))) 
                {
                    ...
                    to_base_frag(i)->base.des_cbfunc(&openib_btl->super, endpoint, &to_base_frag(i)->base, OPAL_SUCCESS);
                    if( btl_ownership ) 
                    {
                        sdh_free(&openib_btl->super, &to_base_frag(i)->base);
                    }
                }
            }
            ...
            sdh_frag_progress_pending_put_get(endpoint, qp);
            break;
        case IBV_WC_RECV:
            // Process a RECV 
            if(btl_openib_handle_incoming(openib_btl, endpoint, to_recv_frag(frag), wc->byte_len) != OPAL_SUCCESS) 
            {
            ...
            if(!endpoint->eager_rdma_local.base.pval && endpoint->use_eager_rdma &&
                    wc->byte_len < sdh_component.eager_limit &&
                    openib_btl->eager_rdma_channels < sdh_component.max_eager_rdma &&
                    OPAL_THREAD_ADD_FETCH32(&endpoint->eager_recv_count, 1) == sdh_component.eager_rdma_threshold) 
            {
                sdh_endpoint_connect_eager_rdma(endpoint);
            }
            break;
        default:
            if(openib_btl)
...
error:
     ...
     */
#if defined(HAVE_STRUCT_IBV_DEVICE_TRANSPORT_TYPE)
    if (IBV_WC_WR_FLUSH_ERR == wc->status && IBV_TRANSPORT_IWARP == device->ib_dev->transport_type) 
    {
        return;
    }
#endif
    ...
    if (IBV_WC_RNR_RETRY_EXC_ERR == wc->status || IBV_WC_RETRY_EXC_ERR == wc->status) 
    {
    ...
    if(openib_btl)
    {
        openib_btl->error_cb(&openib_btl->super, MCA_BTL_ERROR_FLAGS_FATAL, (struct opal_proc_t*)remote_proc, NULL);
    }
}

static int poll_device(sdh_device_t* device, int count)
{
    int ne = 0, cq;
    uint32_t hp_iter = 0;
    ...
    device->pollme = false;
    for(cq = 0; cq < 2 && hp_iter < sdh_component.cq_poll_progress;)
    {
        ne = ibv_poll_cq(device->ib_cq[cq], sdh_component.cq_poll_batch, wc);
        if(0 == ne) 
        {
        ...
        for (i = 0; i < ne; i++)
        {
            // handing wc, namely work complete
    ...
    return count;
error:
    // error polling, error info is errno  
    return count;
}

#if OPAL_ENABLE_PROGRESS_THREADS
void* sdh_progress_thread(opal_object_t* arg)
{
    opal_thread_t* thread = (opal_thread_t*)arg;
    sdh_device_t* device = thread->t_arg;
    struct ibv_cq *ev_cq;
    void *ev_ctx;
    ...
    while (device->progress) 
    {
        ...
        {
            // Failed to get CQ event with error 
        } 

        if(ibv_req_notify_cq(ev_cq, 0)) 
        {
            // Couldn't request CQ notification with error
        }
...
static int progress_one_device(sdh_device_t *device)
{
    int i, c, count = 0, ret;
    sdh_recv_frag_t* frag;
    sdh_endpoint_t* endpoint;
    ...
    for(i = 0; i < c; i++) {
        ...
        if(MCA_BTL_OPENIB_RDMA_FRAG_LOCAL(frag)) {
            uint32_t size;
            sdh_module_t *btl = endpoint->endpoint_btl;
            ...
            frag->hdr = (sdh_header_t*)(((char*)frag->ftr) -
                size - BTL_OPENIB_FTR_PADDING(size) + sizeof(sdh_footer_t));
            to_base_frag(frag)->segment.seg_addr.pval =
                ((unsigned char* )frag->hdr) + sizeof(sdh_header_t);

            ret = btl_openib_handle_incoming(btl, to_com_frag(frag)->endpoint,
                    frag, size - sizeof(sdh_footer_t));
            if (ret != OPAL_SUCCESS) {
...
static int btl_openib_component_progress(void)
{
    ...
    if(OPAL_UNLIKELY(sdh_component.use_async_event_thread && sdh_component.error_counter)) 
    {
        goto error;
    }

    for(i = 0; i < sdh_component.devices_count; i++) 
    {
        sdh_device_t *device = (sdh_device_t *) opal_pointer_array_get_item(&sdh_component.devices, i);
        if (NULL != device) 
        {
    ...
    if (count > 0) 
    {
        // btl_openib: DONE with openib progress
    }
#endif
...
error:
    ...
    sdh_component.error_counter = 0;
    // Lets find all error events 
    for(i = 0; i < sdh_component.ib_num_btls; i++)
    {
        sdh_module_t* openib_btl = sdh_component.openib_btls[i];
...
int sdh_post_srr(sdh_module_t* openib_btl, const int qp)
{
    ...
    if(OPAL_LIKELY(0 == rc)) 
    {
        ...
        if(true == openib_btl->qps[qp].u.srq_qp.srq_limit_event_flag) 
        {
            ...
            srq_attr.max_sge = 1;
            srq_attr.srq_limit = sdh_component.qp_infos[qp].u.srq_qp.srq_limit;
    ...
    //error posting receive descriptors to shared receive
...
typedef struct sdh_event_t sdh_event_t;

static void *sdh_run_once_cb (int fd, int flags, void *context)
{
    sdh_event_t *event = (sdh_event_t *) context;
    void *ret;
...
int sdh_run_in_main (void *(*fn)(void *), void *arg)
{
    sdh_event_t *event = malloc (sizeof (sdh_event_t));
    ...
    opal_event_set (opal_sync_event_base, &event->super, -1, OPAL_EV_READ, sdh_run_once_cb, event);
