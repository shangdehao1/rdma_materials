...
static void sdh_proc_btl_construct(sdh_proc_btlptr_t* elem);
static void sdh_proc_btl_destruct(sdh_proc_btlptr_t* elem);

OBJ_CLASS_INSTANCE(sdh_proc_btlptr_t, opal_list_item_t, sdh_proc_btl_construct, sdh_proc_btl_destruct);
...
static void sdh_proc_construct(sdh_proc_t* proc);
static void sdh_proc_destruct(sdh_proc_t* proc);

OBJ_CLASS_INSTANCE(sdh_proc_t, opal_list_item_t, sdh_proc_construct, sdh_proc_destruct);

void sdh_proc_construct(sdh_proc_t* ib_proc)
{
    ib_proc->proc_opal           = 0;
...
void sdh_proc_destruct(sdh_proc_t* ib_proc)
{
    /* release resources */
 ...
 */
static sdh_proc_t* ibproc_lookup_no_lock(opal_proc_t* proc)
{
    sdh_proc_t* ib_proc;

    OPAL_LIST_FOREACH(ib_proc, &sdh_component.ib_procs, sdh_proc_t) 
    {
...
static sdh_proc_t* ibproc_lookup_and_lock(opal_proc_t* proc)
{
    sdh_proc_t* ib_proc;

    /* get the process from the list */
    opal_mutex_lock(&sdh_component.ib_lock);
    ib_proc = ibproc_lookup_no_lock(proc);
    opal_mutex_unlock(&sdh_component.ib_lock);
    if( NULL != ib_proc )
    {
...
sdh_proc_t* sdh_proc_get_locked(opal_proc_t* proc)
{
    ...
    OPAL_MODEX_RECV(rc, &sdh_component.super.btl_version,
    ...
    if (ib_proc->proc_port_count > 0) {
        ib_proc->proc_ports = (sdh_proc_modex_t *)
            malloc(sizeof(sdh_proc_modex_t) *
    ...
    if (NULL == ib_proc_ret) {
        ...
        opal_mutex_lock(&ib_proc->proc_lock);
        opal_list_append(&sdh_component.ib_procs, &ib_proc->super);
        ib_proc_ret = ib_proc;
        is_new = true;
    } else {
     ...
     * won't lock sdh_component.ib_lock */
    if( !is_new ){
...
err_exit:
    ...
    BTL_ERROR(("%d: error exit from sdh_proc_create", OPAL_PROC_MY_NAME.vpid));
...
int sdh_proc_remove(opal_proc_t *proc, mca_btl_base_endpoint_t *endpoint)
{
    size_t i;
    sdh_proc_t* ib_proc = NULL;
 ...
 * it an address.
 */
int sdh_proc_insert(sdh_proc_t* module_proc, mca_btl_base_endpoint_t* module_endpoint)
{
...
int sdh_proc_reg_btl(sdh_proc_t* ib_proc, sdh_module_t* openib_btl)
{
    sdh_proc_btlptr_t* elem;

    OPAL_LIST_FOREACH(elem, &ib_proc->openib_btls, sdh_proc_btlptr_t) 
    {
    ...
    elem = OBJ_NEW(sdh_proc_btlptr_t);
    if( NULL == elem )
    {
