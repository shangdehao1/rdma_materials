...
static osdh_connect_base_component_t *all[] = {
    // maybe aim to compabile with old code.
...
static osdh_connect_base_component_t *available[5];
...
int osdh_connect_base_register(void)
{
    ...
    if (NULL != btl_openib_cpc_include) {
        sdh_component.cpc_explicitly_defined = true;
...
int osdh_connect_base_select_for_local_port(sdh_module_t *btl)
{
    ...
    cpcs = (osdh_connect_base_module_t **) calloc(num_available,
                  sizeof(osdh_connect_base_module_t *));
    if (NULL == cpcs) {
 ...
 */
int
osdh_connect_base_find_match(sdh_module_t *btl,
                                        sdh_proc_modex_t *peer_port,
                                        osdh_connect_base_module_t **ret_local_cpc,
                                        osdh_connect_base_module_data_t **ret_remote_cpc_data)
{
    int i, j, max = -1;
    osdh_connect_base_module_t *local_cpc, *local_selected = NULL;
    osdh_connect_base_module_data_t *local_cpcd, *remote_cpcd,
 ...
 */
int osdh_connect_base_get_cpc_index(opal_btl_openib_connect_base_component_t *cpc)
{
    int i;
    for (i = 0; NULL != all[i]; ++i) {
        if (all[i] == cpc) {
...
osdh_connect_base_get_cpc_byindex(uint8_t index)
{
...
int osdh_connect_base_alloc_cts(mca_btl_base_endpoint_t *endpoint)
{
    opal_free_list_item_t *fli;
    int length = sizeof(sdh_header_t) +
        sizeof(sdh_header_coalesced_t) +
        sizeof(sdh_control_header_t) +
        sizeof(sdh_footer_t) +
        sdh_component.qp_infos[sdh_component.credits_qp].size;
    ...
    OBJ_CONSTRUCT(&(endpoint->endpoint_cts_frag), sdh_recv_frag_t);
    endpoint->endpoint_cts_frag.super.super.base.order =
        sdh_component.credits_qp;
...
int osdh_connect_base_free_cts(mca_btl_base_endpoint_t *endpoint)
{
        ...
        mca_btl_base_endpoint_t *endpoint)
{
    ...
    if (cpc->cbm_uses_cts) {
        int rc;
        rc = osdh_connect_base_alloc_cts(endpoint);
        if (OPAL_SUCCESS != rc) {
            return rc;
        }
    }
