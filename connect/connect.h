 ...
 * Hence, these entities are referred to as "
 *
 * =========== Connect Pseudo-Components" (CPCs) ================
 *
 ...
 *
 * - component_register(): 
 *   The openib BTL's component_open() function
 ...
 *
 * - component_init(): 
 *   The openib BTL's component_init() function
 ...
 *
 * - component_query(): 
 *   The openib BTL's init_one_port() calls the
 ...
 *
 * component_finalize(): 
 *   The openib BTL's component_close() function
 ...
 * ======== Module interface: ============
 *
 * cbm_component member: 
 *   A pointer pointing to the single, global
 ...
 *
 * cbm_modex_message member: 
 *   A pointer to a blob buffer that will be
 ...
 *
 * cbm_uses_cts: 
 *   a bool that indicates whether the CPC will use the CTS protocol or not.
 *   - if true: the CPC will post the fragment on
 *     endpoint->endpoint_cts_frag as a receive buffer and will *not*
 *     call osdh_post_recvs().
 *   - if false: the CPC will call osdh_post_recvs() before
 *     calling osdh_cpc_complete().
 *
 * There are two functions in the main openib BTL that the CPC may
 * call:
 *
 * - osdh_post_recvs(endpoint): 
 *     once a QP is locally
 ...
 *
 * - osdh_cpc_complete(endpoint): 
 *     once that a CPC knows
 ...
 *   being ignored).
 */
typedef int (*osdh_connect_base_component_init_fn_t)(void);
 ...
 */
typedef int (*osdh_connect_base_func_component_query_t)
    (struct sdh_module_t *btl,
     struct osdh_connect_base_module_t **cpc);

/**
 * This function is invoked once by the openib BTL component during
 * shutdown.  It is intended to have CPC component-wide shutdown.
 */
typedef int (*osdh_connect_base_component_finalize_fn_t)(void);

// =================== CPC component struct ========================
//
// all member is function pointer..
struct osdh_connect_base_component_t {
    // Name of this set of connection functions 
    char cbc_name[BCF_MAX_NAME];
    // Register function. Can be NULL. 
    osdh_connect_base_component_register_fn_t cbc_register;
    // CPC component init function.  Can be NULL. 
    osdh_connect_base_component_init_fn_t cbc_init;
    // Query the CPC component to get a CPC module corresponding to
    // an openib BTL module.  Cannot be NULL. 
    osdh_connect_base_func_component_query_t cbc_query;
    // CPC component finalize function.  Can be NULL. 
    osdh_connect_base_component_finalize_fn_t cbc_finalize;
};

// Convenience typedef
typedef struct osdh_connect_base_component_t osdh_connect_base_component_t;

// ======================== define function type =========================

// Function called when an endpoint has been created and has been
// associated with a CPC.
typedef int (*osdh_connect_base_module_endpoint_init_fn_t)
    (struct mca_btl_base_endpoint_t *endpoint);

// Function to initiate a connection to a remote process.
typedef int (*osdh_connect_base_module_start_connect_fn_t)
    (struct osdh_connect_base_module_t *cpc,
     struct mca_btl_base_endpoint_t *endpoint);

// Function called when an endpoint is being destroyed.
typedef int (*osdh_connect_base_module_endpoint_finalize_fn_t)
     (struct mca_btl_base_endpoint_t *endpoint);
...
typedef int (*osdh_connect_base_module_finalize_fn_t)
    (struct sdh_module_t *btl,
     struct osdh_connect_base_module_t *cpc);

// ====================== Meta data about a CPC module.==========================
//
// This is in a standalone struct because it is used in both the CPC module struct and 
// the openib_btl_proc_t struct to hold information received from the
// modex.
typedef struct osdh_connect_base_module_data_t {
    // Pointer back to the component. 
    // Used by the base and openib btl to calculate this module's index for the modex.
    osdh_connect_base_component_t *cbm_component;
...
// =========================================================================
//
typedef struct osdh_connect_base_module_t {
    // Meta data about the module 
    osdh_connect_base_module_data_t data;

    // Endpoint initialization function 
    osdh_connect_base_module_endpoint_init_fn_t cbm_endpoint_init;

    // Connect function 
    osdh_connect_base_module_start_connect_fn_t cbm_start_connect;

    // Endpoint finalization function 
    osdh_connect_base_module_endpoint_finalize_fn_t cbm_endpoint_finalize;

    // Finalize the cpc module 
    osdh_connect_base_module_finalize_fn_t cbm_finalize;

    /** Whether this module will use the CTS protocol or not.  This
        directly states whether this module will call
        sdh_endpoint_post_recvs() or not: true = this
